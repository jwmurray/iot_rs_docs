...
async fn main(spawner: Spawner) {
    ...
    unwrap!(spawner.spawn(print_periodic_message(Duration::from_secs(5), "Info thread")));
}

#[embassy_executor::task(pool_size = 1)]
async fn print_periodic_message(delay: Duration, name: &'static str) {
    let mut info_ticker = Ticker::every(delay); // Ticker for 5 seconds
    info!("Starting {} ", name);
    let mut count = 0;
    loop {
        {
             count += 5;
            info!("{} is still running, count: {}", name, count);
        }
        info_ticker.next().await;
    }
}
